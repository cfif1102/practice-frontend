...
import { createEquipments, findOneEquipment, updateEquipments } from '@thunks';
...
import { CreateEquipemnt, Equipment } from '@@types';
...
const EditEquipmentForm: FC<Props> = ({ entity, id }) => {
  ...
  const formOnSubmit = (data: FormData) => {
    dispatch(updateEquipments({ id, equipment: data }));
  };

  useEffect(() => {
    dispatch(equipmentsActions.setName(entity.name));
    dispatch(equipmentsActions.setManufacturer(entity.manufacturer));
    dispatch(equipmentsActions.setType(entity.type));
    dispatch(equipmentsActions.setModel(entity.model));
    dispatch(equipmentsActions.setSerialNumber(entity.serialNumber));
    dispatch(equipmentsActions.setInnovationNumber(entity.innovationNumber));
    dispatch(equipmentsActions.setWorkhours(entity.workHours));
    dispatch(equipmentsActions.setWorkshopId(entity.workshop.id));

    setValue('name', entity.name);
    setValue('manufacturer', entity.manufacturer);
    setValue('type', entity.type);
    setValue('model', entity.model);
    setValue('serialNumber', entity.serialNumber);
    setValue('innovationNumber', entity.innovationNumber);
    setValue('workHours', entity.workHours);
    setValue('workshopId', entity.workshop.id);
  }, [entity]);

  useThunkRedirect(updateEquipmentThunk, '/equipments', equipmentsActions.restoreUpdateThunk);
    ...
    <WithLoader status={status}>
      <FormWrapper>
        <FormStyled onSubmit={handleSubmit(formOnSubmit)} height="auto">
          ...
          <WithLoader status={updateEquipmentThunk.status}>
...
export default withCheckId(
  'id',
  (state: RootState) => state.equipments.findOneThunk,
  findOneEquipment,
  (state: RootState) => state.equipments.equipment,
)(EditEquipmentForm);
